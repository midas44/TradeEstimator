...
namespace TradeEstimator.Trade
{
    public class Input
    {
        ...
        public Input(Config config, Logger logger, TradeModel trModel, string timestamp, string instrument)
        {
            this.config = config;
            this.logger = logger;
        ...
        private void load(string path)
        {
            ...
            foreach (string line in dataset)
            {
                ...
                size.Add(int.Parse(s[0], CultureInfo.InvariantCulture));
                price.Add(double.Parse(s[0], CultureInfo.InvariantCulture));
